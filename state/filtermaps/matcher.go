...
func isLvIndexInRange(lvIndex uint64, rg *lvIndexRange) bool {
	...
	return lvIndex >= rg.startLvIndex && lvIndex < rg.txEventsPointers[len(rg.txEventsPointers)-1]
}
...
func lvIndexInRange(lvIndex uint64, rg *lvIndexRange) (int, error) {
	if rg == nil {
		...
		return -1, nil
	} else if lvIndex >= rg.txEventsPointers[len(rg.txEventsPointers)-1] {
		return 1, nil
	} else {
		return 0, nil
	}
...
func getTxIndexInRange(lvIndex uint64, rg *lvIndexRange) (uint64, error) {
	...
	for i, txEventsPointer := range rg.txEventsPointers {
		if lvIndex >= startLvIndex && lvIndex < txEventsPointer {
			return uint64(i), nil
		}
		startLvIndex = txEventsPointer
	}
	return 0, fmt.Errorf("log value index %d is not in range %d-%d", lvIndex, rg.startLvIndex, rg.txEventsPointers[len(rg.txEventsPointers)-1])
}
