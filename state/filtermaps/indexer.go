...
func (f *FilterMaps) indexerLoop() {
	...
	f.logger.Info("Started log indexer")
	...
	for !f.stop {
		...
		if !f.indexedRange.initialized {
			...
			if err := f.init(); err != nil {
				...
				err = f.reset() // remove broken index from DB
				if err != nil {
					f.logger.Error("Failed to reset filtermaps", "error", err)
				}
				return
...
func (f *FilterMaps) disableForError(op string, err error) {
	f.logger.Error("Log index "+op+" failed, reverting to unindexed mode", "error", err)
	f.disabled = true
	close(f.disabledCh)
}
...
func (f *FilterMaps) tryIndexHead() error {
	...
	if _, err := headRenderer.run(func() bool {
		...
		if f.indexedRange.hasIndexedBlocks() && f.indexedRange.blocks.AfterLast() >= f.ptrHeadIndex &&
			...
			f.logger.Info("Log index head rendering in progress",
				...
				"remaining", f.indexedHeight-f.indexedRange.blocks.Last(),
	...
	if f.loggedHeadIndex && f.indexedRange.hasIndexedBlocks() {
		f.logger.Info("Log index head rendering finished",
			"firstblock", f.indexedRange.blocks.First(), "lastblock", f.indexedRange.blocks.Last(),
			"processed", f.indexedRange.blocks.AfterLast()-f.ptrHeadIndex,
			"elapsed", common.PrettyDuration(time.Since(f.startedHeadIndexAt)))
	}
...
func (f *FilterMaps) tryIndexTail() (bool, error) {
	for {
		...
		if !f.startedTailIndex {
			...
			if f.indexedRange.hasIndexedBlocks() && f.ptrTailIndex >= f.indexedRange.blocks.First() &&
				...
				f.logger.Info("Log index tail rendering in progress",
	...
	if f.loggedTailIndex && f.indexedRange.hasIndexedBlocks() {
		f.logger.Info("Log index tail rendering finished",
			"firstblock", f.indexedRange.blocks.First(), "lastblock", f.indexedRange.blocks.Last(),
...
func (f *FilterMaps) tryUnindexTail() (bool, error) {
	...
	if f.startedTailUnindex && f.indexedRange.hasIndexedBlocks() {
		f.logger.Info("Log index tail unindexing finished",
...
func (f *FilterMaps) needTailEpoch(epoch uint32) bool {
	...
	if err != nil {
		f.logger.Error("Could not get last block of epoch", "epoch", epoch, "error", err)
		return epoch >= firstEpoch
	}
...
func (f *FilterMaps) tailTargetBlock() uint64 {
	if f.history == 0 || f.indexedHeight < f.history {
		return 0
	}
	return f.indexedHeight + 1 - f.history
}
...
func (f *FilterMaps) tailPartialBlocks() uint64 {
	...
	end, err := f.getLastBlockOfMap(f.indexedRange.maps.First() - f.mapsPerEpoch + f.indexedRange.tailPartialEpoch - 1)
	if err != nil {
		f.logger.Error("Error fetching last block of map", "mapIndex", f.indexedRange.maps.First()-f.mapsPerEpoch+f.indexedRange.tailPartialEpoch-1, "error", err)
	}
	var start uint64
	if f.indexedRange.maps.First()-f.mapsPerEpoch > 0 {
		start, err = f.getLastBlockOfMap(f.indexedRange.maps.First() - f.mapsPerEpoch - 1)
		if err != nil {
			f.logger.Error("Error fetching last block of map", "mapIndex", f.indexedRange.maps.First()-f.mapsPerEpoch-1, "error", err)
		}
	}
	return end - start
}
...
func (f *FilterMaps) targetHeadIndexed() bool {
	return f.targetHeight == f.indexedHeight && f.indexedRange.headIndexed
}
