...
func (f *FilterMaps) renderMapsFromSnapshot(cp *renderedMap) (*mapRenderer, error) {
	...
	iter, err := f.newLogIteratorFromBlockDelimiter(cp.lastBlock, cp.headDelimiter, f.stateStore)
	if err != nil {
...
func (f *FilterMaps) renderMapsFromMapBoundary(firstMap, renderBefore uint32, startBlock, startLvPtr uint64) (*mapRenderer, error) {
	iter, err := f.newLogIteratorFromMapBoundary(firstMap, startBlock, startLvPtr, f.stateStore)
	if err != nil {
...
func (f *FilterMaps) lastCanonicalSnapshotOfMap(mapIndex uint32) *renderedMap {
	...
	for _, blockNumber := range f.renderSnapshots.Keys() {
		if cp, _ := f.renderSnapshots.Get(blockNumber); cp != nil && blockNumber < f.indexedRange.blocks.AfterLast() &&
			blockNumber <= f.indexedHeight &&
			blockNumber <= f.targetHeight &&
...
func (f *FilterMaps) loadHeadSnapshot() error {
	...
	lastBlock, err := f.getLastBlockOfMap(f.indexedRange.maps.Last())
	if err != nil {
...
func (r *mapRenderer) makeSnapshot() {
	if r.iterator.blockNumber != r.currentMap.lastBlock || r.iterator.targetHeight != r.f.targetHeight {
...
func (r *mapRenderer) writeFinishedMaps(pauseCb func() bool) error {
	...
	renderedHeight := r.f.targetHeight // pauseCb callback might still change targetView while writing finished maps
	...
	checkWriteCnt := func() {
		...
		if writeCnt == rowsPerBatch {
			...
			if err := batch.Write(); err != nil {
				r.f.logger.Error("Error writing log index update batch", "error", err)
			}
	...
	if tempRange != r.f.indexedRange {
		err = r.f.setRange(batch, r.f.indexedHeight, tempRange, true)
		if err != nil {
			return fmt.Errorf("failed to set temporary range: %v", err)
		}
	}
	...
	if r.finished.First() > 0 {
		...
		lastBlock, err := r.f.getLastBlockOfMap(r.finished.First() - 1)
		if err != nil {
	...
	err = r.f.setRange(batch, renderedHeight, newRange, false)
	if err != nil {
		return fmt.Errorf("failed to set range: %v", err)
	}
	if err = batch.Write(); err != nil {
...
func (r *mapRenderer) getTempRange() (filterMapsRange, error) {
	...
	if tempRange.maps.First() != r.f.indexedRange.maps.First() {
		...
		if tempRange.maps.First() > 0 {
			firstBlock, err := r.f.getLastBlockOfMap(tempRange.maps.First() - 1)
			if err != nil {
	...
	if tempRange.maps.AfterLast() != r.f.indexedRange.maps.AfterLast() {
		...
		if !tempRange.maps.IsEmpty() {
			lastBlock, err := r.f.getLastBlockOfMap(tempRange.maps.Last())
			if err != nil {
...
func (r *mapRenderer) getUpdatedRange() (filterMapsRange, error) {
	...
	if newRange.maps.First() != r.f.indexedRange.maps.First() {
		...
		if newRange.maps.First() > 0 {
			firstBlock, err := r.f.getLastBlockOfMap(newRange.maps.First() - 1)
			if err != nil {
	...
	if newRange.maps.AfterLast() == r.finished.AfterLast() {
		...
		if lm.finished {
			newRange.blocks.SetLast(r.f.targetHeight)
			if lm.lastBlock != r.f.targetHeight {
		...
		if r.f.indexedHeight != r.f.targetHeight {
...
// current targetView.
func (f *FilterMaps) newLogIteratorFromBlockDelimiter(blockNumber, lvIndex uint64, stateStore sm.Store) (*logIterator, error) {
	if blockNumber > f.targetHeight {
		return nil, fmt.Errorf("iterator entry point %d after target chain head block %d", blockNumber, f.targetHeight)
	}
...
func (f *FilterMaps) newLogIteratorFromMapBoundary(mapIndex uint32, startBlock, startLvPtr uint64, stateStore sm.Store) (*logIterator, error) {
	if startBlock > f.targetHeight {
...
func (l *logIterator) updateChainHeight(targetHeight uint64) {
	l.targetHeight = targetHeight
}
...
func (l *logIterator) next() error {
	...
	if l.delimiter {
		...
		l.txEventsPointers = make([]uint64, len(blockResponse.TxResults))
		for i, txResult := range blockResponse.TxResults {
			for _, event := range txResult.Events {
				l.txEventsPointers[i] += uint64(len(event.Attributes))
			}
		}
...
func (l *logIterator) enforceValidState() {
	...
	if l.blockNumber == l.targetHeight {
		l.finished = true
	} else {
		l.delimiter = true
	}
