...
func (fm *FilterMapsMatcherBackend) GetLogByLvIndex(ctx context.Context, lvIndex uint64) (*TxEvent, error) {
	fm.f.indexLock.RLock()
	defer fm.f.indexLock.RUnlock()
...
func (fm *FilterMapsMatcherBackend) GetLvIndexRange(ctx context.Context, lvIndex uint64) (lvIndexRange, error) {
	fm.f.indexLock.RLock()
	defer fm.f.indexLock.RUnlock()

	return fm.f.getLvIndexRange(lvIndex)
}
...
func (fm *FilterMapsMatcherBackend) synced() {
	...
	fm.syncCh <- SyncRange{
		IndexedHeight: fm.f.indexedHeight,
...
func (fm *FilterMapsMatcherBackend) SyncLogIndex(ctx context.Context) (SyncRange, error) {
	...
	case <-fm.f.disabledCh:
		...
		return SyncRange{IndexedHeight: fm.f.indexedHeight}, nil
	}
	select {
	case vr := <-syncCh:
		return vr, nil
